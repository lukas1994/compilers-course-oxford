...
let has_value d = 
  match d.d_kind with
      ConstDef _ | VarDef | RegDef | CParamDef 
        | VParamDef | StringDef -> true 
    | _ -> false
      ...
      Variable x ->
        ...
        begin
          match d.d_kind with 
            ...
            | RegDef -> 
                if addressible then
                  sem_error "register variable is not allowed here" []
            | _ -> 
    ...
    let t = match e.e_guts with
        Variable x -> 
          ...
          if not (has_value d) then
            sem_error "$ is not a variable" [fId x.x_name];
          if d.d_kind = RegDef && d.d_level < !level then
            sem_error "access to register variable from nested scope" [];
          begin
            match d.d_kind with 
...
let local_alloc level size nreg d =
  if d.d_kind = RegDef && scalar (d.d_type) 
...
and check_decls alloc env ds = List.fold_left (check_decl alloc) env ds
...
let rec check_block level rt env (Block (ds, ss, fsize, nregv)) =
  ...
  let env' = check_decls alloc (new_block env) ds in
